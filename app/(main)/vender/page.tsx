...
import { IconCamera, IconTag, IconRuler, IconPackage, IconCheck, IconX, IconLoader2 } from "@tabler/icons-react";
...
export default function VenderPage() {
  ...
  const handlePublish = async () => {
    if (!titulo || !precio || !categoria || !estado || !talla) {
    ...
    try {
      ...
      const uploaded = await Promise.all(
        imagenes.map(img => uploadImage(img.file, "xochic/prendas"))
      );
      const urls = uploaded.map(u => u.secure_url);
      const ids  = uploaded.map(u => u.public_id);

      // 2. Guardar prenda en Firestore
      await crearPrenda({
        titulo, descripcion: desc, precio: Number(precio),
        talla, categoria, estilos, estado,
        imagenes: urls, imagenesIds: ids,
        medidas: {},
  ...
  const chip = (active: boolean, color?: string) => ({
    background: active ? (color ? color + "33" : "var(--primary)") : "var(--muted)",
    color:      active ? (color ? color       : "white")           : "var(--muted-foreground)",
    border:     active ? `1.5px solid ${color ?? "var(--primary)"}` : "1.5px solid transparent",
    borderRadius: 999, padding: "6px 14px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer",
  ...
  if (published) return (
    <div style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <p className="text-sm text-center" style={{ color: "var(--muted-foreground)" }}>
        Ya esta visible en el marketplace. Te notificaremos cuando alguien la compre.
      </p>
    </div>
  );
  ...
  return (
    <div className="px-4 pb-8 mt-2">
      ...
      <p className="text-sm mb-5" style={{ color: "var(--muted-foreground)" }}>Comision Xochic: solo 9% — una de las mas bajas del mercado</p>
      ...
      <div className="grid grid-cols-4 gap-2 mb-5">
        ...
        {imagenes.length < 8 && (
          <button onClick={() => fileRef.current?.click()}
            ...
            style={{ aspectRatio: "1", border: "2px dashed var(--primary)", background: "var(--muted)" }}>
            <IconCamera size={22} style={{ color: "var(--primary)" }} />
            <span className="text-xs" style={{ color: "var(--primary)" }}>
              {imagenes.length === 0 ? "Agregar fotos" : "+"}
            </span>
          </button>
        )}
      </div>
      ...
      <div className="mb-4">
        ...
        <textarea value={desc} onChange={e => setDesc(e.target.value)} rows={3}
          placeholder="Marca, materiales, detalles importantes..."
      ...
      <div className="mb-4">
        ...
        <input type="number" value={precio} onChange={e => setPrecio(e.target.value)} placeholder="0"
          ...
          style={{ background: "var(--input)", border: "1.5px solid var(--border)", color: "var(--foreground)", outline: "none" }} />
        {precio && (
          <div className="mt-2 p-3 rounded-xl space-y-1" style={{ background: "var(--muted)" }}>
            <div className="flex justify-between text-xs">
              <span style={{ color: "var(--muted-foreground)" }}>Precio de venta</span>
              <span style={{ color: "var(--foreground)" }}>${Number(precio).toLocaleString()} MXN</span>
            </div>
            <div className="flex justify-between text-xs">
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {CATEGORIAS.map(c => (
            <button key={c} onClick={() => setCategoria(c)} style={chip(categoria === c)}>{c}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {TALLAS.map(t => (
            <button key={t} onClick={() => setTalla(t)} style={chip(talla === t)}>{t}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mb-4">
        <label className="block text-xs font-semibold mb-2" style={{ color: "var(--foreground)" }}>
          Estilo <span style={{ color: "var(--muted-foreground)", fontWeight: 400 }}>(max 2)</span>
        </label>
        <div className="flex flex-wrap gap-2">
          {ESTILOS.map(e => (
            <button key={e} onClick={() => toggleEstilo(e)} style={chip(estilos.includes(e))}>{e}</button>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {ESTADOS.map(e => (
            <button key={e} onClick={() => setEstado(e)}
              style={chip(estado === e, estado === e ? "var(--success)" : undefined)}>
      ...
      <div className="mb-5 p-3 rounded-2xl flex items-center gap-3" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>Al venderse generamos tu guia de envio. La compradora paga el envio.</p>
        </div>
      </div>

      {/* ── Publicar ── */}
      <button onClick={handlePublish} disabled={uploading || !titulo || !precio || !categoria || !estado || !talla}
        className="w-full py-4 rounded-2xl font-bold text-base flex items-center justify-center gap-2"
        style={{
          background: titulo && precio && categoria && estado && talla ? "var(--primary)" : "var(--muted)",
          color: titulo && precio && categoria && estado && talla ? "white" : "var(--muted-foreground)",
          opacity: uploading ? 0.8 : 1,
        }}>
        {uploading ? <><IconLoader2 size={20} className="animate-spin" /> Subiendo...</> : "Publicar prenda"}
      </button>
    </div>
  );
}
